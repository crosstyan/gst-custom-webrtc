...
static gboolean bus_watch_cb(GstBus *bus, GstMessage *message,
                             gpointer user_data) {
...
ReceiverEntry *create_receiver_entry(SoupWebsocketConnection *connection) {
  ...
  receiver_entry->pipeline = gst_parse_launch(
      ...
      "video/x-raw,width=640,height=360,framerate=15/1 ! videoconvert ! queue "
      "max-size-buffers=1 ! x264enc bitrate=600 speed-preset=ultrafast "
      ...
      "application/"
      "x-rtp,media=video,encoding-name=H264,payload=" RTP_PAYLOAD_TYPE
      " ! webrtcbin. "
      "autoaudiosrc ! queue max-size-buffers=1 leaky=downstream"
      " ! audioconvert ! audioresample ! opusenc  ! rtpopuspay "
      "pt=" RTP_AUDIO_PAYLOAD_TYPE " ! application/x-rtp, encoding-name=OPUS !"
      " webrtcbin. ",
      &error);
  if (error != NULL) {
...
int main(int argc, char *argv[]) {
  ...
  receiver_entry_table = g_hash_table_new_full(g_direct_hash, g_direct_equal,
  ...
  soup_server_add_websocket_handler(soup_server, "/ws", NULL, NULL,
                                    ...
                                    (gpointer)receiver_entry_table, NULL);
