...
static void data_channel_on_message_string(GObject *dc, gchar *str,
                                           gpointer user_data) {
...
static gboolean on_webrtcbin_stat(GQuark field_id, const GValue *value,
                                  gpointer unused) {
...
static gboolean start_pipeline(gboolean create_offer, guint opus_pt,
                               guint vp8_pt) {
  char *pipeline;
  ...
  pipeline = g_strdup_printf(
      ...
      "! queue ! opusenc ! rtpopuspay name=audiopay ! "
      "queue ! %s,payload=%u ! sendrecv. ",
...
static gboolean check_plugins(void) {
  ...
  const gchar *needed[] = {"opus",         "vpx",  "nice",       "webrtc",
                           "dtls",         "srtp", "rtpmanager", "videotestsrc",
                           "audiotestsrc", NULL};
